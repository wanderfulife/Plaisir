...
// Firestore refs
const surveyCollectionRef = collection(db, "Plaisir");
const counterDocRef = doc(db, "counterPlaisir", "surveyCounter");
...
const startMessage = computed(() => {
  return "Bonjour, Saint-Quentin-en-Yvelines et Île-de-France Mobilités  souhaiteraient connaitre vos habitudes de déplacement sur ce secteur, 	Accepteriez-vous de répondre à 4 questions ? 	";
});
...
const getNextId = async () => {
  ...
  return `Plaisir-${counter.toString().padStart(6, "0")}`;
};
</script>
